<script lang="ts">
  ...
  import type { Subject } from "$lib/features/exams/types";
  ...
  function formatRelativeTime(isoString: string) {
    ...
    if (daysDifference === 0) {
      const hoursDiff = Math.round(
        (new Date(isoString).getTime() - Date.now()) / (1000 * 60 * 60),
      );
...
<section class="mb-12">
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
    {#each subjects.slice(0, 3) as subject}
      <Card.Root
        class="flex flex-col hover:border-primary/50 transition-colors group"
      >
        <Card.Header class="pb-3">
          <Card.Title class="line-clamp-1">{subject.name}</Card.Title>
        </Card.Header>
        <Card.Content class="pb-2 grow">
          <div class="flex justify-between text-sm mb-2">
            <span class="text-muted-foreground"
              >{subject.attemptedQuestions} / {subject.totalQuestions} questions</span
            >
